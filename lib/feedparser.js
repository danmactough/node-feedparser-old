  ...
  var parser = new xml.SaxParser(function(cb) {
    ...
    cb.onEndElementNS(function(elem, prefix, uri) {
      var el = ( prefix ? prefix.toLowerCase() + ':' : '' ) + elem.toLowerCase()
        , node = nodes.shift();
        ;
      if (el == xhtml['el']) { // The end of the XHTML
        ...
        xhtml = {};
        in_xhtml = false;
      } else if (in_xhtml) { // Somewhere in the middle of the XHTML
        xhtml['value'] += '</' + el + '>';
      } else if(elem.toLowerCase() == 'item' || elem.toString() == 'entry') { // The end of an item or entry
          in_item = false;
          article_count ++;
        ...
        */
        ...
        switch(el) {
          ...
          default:
            ...
            //if (Array.isArray(channel_nodes[0])) { // We are not nested because we haven't mutated nodes[0] yet
            /*if (nodes[1]['el'] != 'channel' && nodes[1]['el'] != 'feed') {
              if (channel_nodes[0][1].length && typeof channel_nodes[0][1] === 'string') {
                channel_nodes[0][1] = [ { '_xmltext': channel_nodes[0][1] }, node ];
              } else {
                channel_nodes[0][1].push(node);
              }
            }
            */
        }
